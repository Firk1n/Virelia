...
"""Rebuild the Virelia map tiles from the Photoshop source in one command.
    ...
    python scripts/build_map.py            # export from the PSD, then re-tile
    python scripts/build_map.py --list     # print the PSD's top-level groups
    python scripts/build_map.py --tiles-only   # skip Photoshop, re-tile the PNGs
    python scripts/build_map.py labels     # only rebuild the labels overlay
...
CONFIG = os.path.join(ROOT, "scripts", "map_layers.json")
STAMPS = os.path.join(ROOT, "scripts", ".build-stamps.json")
